...
function App() {
  ...
  return (
    <>
      ...
      {/* Floating hearts */}
      <FloatingHearts accentColor={currentTrack.color} isPlaying={isPlaying} />

      {/* Sparkles */}
      <Sparkles accentColor={currentTrack.color} isPlaying={isPlaying} />
      ...
      <div className="min-h-screen flex flex-col relative" style={{ background: 'var(--bg)' }}>
        ...
        <div className="flex-1 flex flex-col lg:flex-row max-w-[1100px] w-full mx-auto">
          ...
          >
            {/* Album art */}
            <div key={`art-${trackKey}`} className="track-enter mb-6">
              <AlbumArt track={currentTrack} isPlaying={isPlaying} />
            </div>

            {/* Track info */}
            <div key={`info-${trackKey}`} className="track-enter mb-5" style={{ animationDelay: '60ms' }}>
