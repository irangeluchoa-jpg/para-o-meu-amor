...
export function useCursor(accentColor?: string) {
  // Update cursor color when accentColor changes
  useEffect(() => {
    if (!accentColor) return
    const dot  = document.getElementById('cursor-dot')
    const ring = document.getElementById('cursor-ring')
    if (dot)  dot.style.background    = accentColor
    if (ring) ring.style.borderColor  = accentColor + '80'
  }, [accentColor])
